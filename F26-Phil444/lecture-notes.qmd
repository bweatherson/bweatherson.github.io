...
1. Assume that every player will make a rational choice given that no player will play a dominated strategy.
...
Alternatively we could just use the formula given above. Just adding up rows gives us the following numbers. $$\begin{align*}
\Pr(p \wedge q) &=& 0.0008 + 0.008 = 0.0088 \\
\Pr(q) &=& 0.0008 + 0.008 + 0.0002 + 0.001 = 0.01
\end{align*}$$ Then we can apply the formula. $$\begin{align*}
...
This is a bit of a digression from what we're really focussing on in game theory, but it's important for understanding how conditional probabilities work. It is often easier to calculate conditional probabilities in the 'inverse' direction to what we are interested in. That is, if we want to know $\Pr(A | B)$, it might be much easier to discover $\Pr(B | A)$. In these cases, we use Bayes Theorem to get the right result. I'll state Bayes Theorem in two distinct ways, then show that the two ways are ultimately equivalent. $$\begin{align*}
\Pr(A|B) &=& \frac{\Pr(B|A) \Pr(A)}{\Pr(B)} \\
&=& \frac{\Pr(B|A) \Pr(A)}{\Pr(B|A)Pr(A) + \Pr(B|\neg A)Pr(\neg A)}
\end{align*}$$ These are equivalent because $\Pr(B) = \Pr(B|A)Pr(A) + \Pr(B|\neg A)\Pr(\neg A)$. Since this is an independently interesting result, it's worth going through the proof of it. First note that $$\begin{align*}
\Pr(B|A)\Pr(A) &=& \frac{\Pr(A \wedge B)}{\Pr(A)} \Pr(A) \\
&=& \Pr(A \wedge B) \\
\\
\Pr(B|\neg A)\Pr(\neg A) &=& \frac{\Pr(\neg A \wedge B)}{\Pr\neg (A)} \Pr(\neg A) \\
&=& \Pr(\neg A \wedge B)
\end{align*}$$ Adding those two together we get $$\begin{align*}
\Pr(B|A)\Pr(A) + \Pr(B|\neg A)\Pr(\neg A) &=& \Pr(A \wedge B) + \Pr(\neg A \wedge B) \\
&=& \Pr((A \wedge B) \vee (\neg A \wedge B)) \\
&=& \Pr(B)
\end{align*}$$ The second line uses the fact that $A \wedge B$ and $\neg A \wedge B$ are inconsistent. And the third line uses the fact that $(A \wedge B) \vee (\neg A \wedge B)$ is equivalent to $A$. So we get a nice result, one that we'll have occasion to use a bit in what follows. $$\begin{equation*}
...
Now we can apply Bayes theorem in its second form. $$\begin{align*}
\Pr(A|B) &=& \frac{\Pr(B|A) \Pr(A)}{\Pr(B|A)\Pr(A) + \Pr(B|\neg A)\Pr(\neg A)} \\
&=& \frac{0.8 \times 0.05}{0.08 \times 0.05 + 0.1 \times 0.95} \\
&=& \frac{0.04}{0.04 + 0.095} \\
&=& \frac{0.04}{0.135} \\
&\approx& 0.296
\end{align*}$$ So in fact the probability of having the disease, conditional on having a positive test, is less than 0.3. So in that sense the test is quite unreliable.
...
1. Finding the probability with which they play each part of their support.
...
1. Imagine that my strategy will leak to the police, no matter what. That could be because I'm so incompetent that I need to rely on public advice, or it could be because the police have a way of reading my moves. And imagine that the police are in the same boat; their strategy will leak to me. Then we should each play the equilibrium strategy; it is immune to doing worse in light of leaks. That's an implausible situation in most cases, but there is one special case worth considering.

1. Imagine the game will be repeated so many times that eventually our strategies will become visible enough just from the record of our play. Then we are (eventually) back into the 'leaked strategy' situation. And in that situation, we should play equilibrium strategies.
...
>
> **[Figure to be redrawn: nine shapes, one salient. Source: `figures/src/focal-point-1.tex`]**

It's not obvious what they should do there, but compare the situation our players face in the following game.

> $A$ and $B$ have to select, without communicating, one of the following nine figures. They each get a reward iff they select the same figure.
>
> **[Figure to be redrawn: nine shapes, one salient. Source: `figures/src/focal-point-2.tex`]**
...
::: {#fig-five .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/five.tex`
...
::: {#fig-pd-tree .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/pd-tree.tex`
...
::: {#fig-five-2 .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/five-2.tex`
...
::: {#fig-five-3 .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/five-3.tex`
...
::: {#fig-five-rev .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/five-rev.tex`
...
::: {#fig-five-rev-2 .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/five-rev-2.tex`
...
::: {#fig-easy-game .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/easy-game.tex`
...
::: {#fig-incredible .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/incredible.tex`
...
::: {#fig-caterpillar .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/caterpillar.tex`
...
1. What to do in the second round if the other player cooperates in the first round.

1. What to do in the second round if the other player defects in the first round.

1. What to do in the third round if the other player cooperates in each of the first two rounds.

1. What to do in the third round if the other player cooperates in the first round then defects in the second round.

1. What to do in the third round if the other player defects in the first round then cooperates in the second round.

1. What to do in the third round is the other player defects in each fo the first two rounds.
...
::: {#fig-perfect .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/perfect.tex`
...
::: {#fig-simple-signal .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/simple-signal.tex`
...
::: {#fig-college-signal .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/college-signal.tex`
...
::: {#fig-canadian-signal .tree-placeholder}
> **Game tree to be redrawn.** Source: `figures/src/canadian-signal.tex`
