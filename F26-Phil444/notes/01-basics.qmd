# Basics of Game Theory {#sec-basics}

## Nature of Games {#sec-nature-of-games}
...
: Prisoners' Dilemma {#tbl-pd-standard}
...
- We can specify their **payouts**. These are the concrete things the players get. 
...
## Four Examples of Games {#sec-four-examples}
...
## Prisoners' Dilemma {#sec-pd}

We've already seen this game, it was @tbl-pd-standard, but it's worthwhile having on the table again.
...
The general form of Prisoners' Dilemma is given in @tbl-pd-abstract, assuming $a > b > c > d$, and $2b > a + d$.
...
: The general form of a Prisoners' Dilemma game {#tbl-pd-abstract}
...
## Stag Hunt {#sec-stag-hunt}

The next game we'll look at is called Stag Hunt. We'll first do a version with commonly used numbers (in @tbl-sh-standard), then the general version.
...
: A familiar version of Stag Hunt {#tbl-sh-standard}

The general version, shown in @tbl-sh-abstract, has $a > b \geqslant c > d$ and $b + c > a + d$. The case where $b = c$ is allowed; that's the case where the rabbit hunter's return doesn't depend on what the other hunter is doing.
...
: The general form of Stag Hunt {#tbl-sh-abstract}

In Stag Hunt, the best outcome for all parties is the cooperative one. But if a player has no idea what the other player will do, and thinks it is 50/50 whether they will do the cooperative thing, then their best bet is to not cooperate.

The cute story behind this game comes from the eighteenth century political philosopher Jean-Jacques Rousseau [@rousseau1755]. Imagine two people are setting out on the day's hunt, and are going to hunt stag or rabbits. Catching a stag will lead to more meat, but it is impossible with just one person. One person can catch a lot of rabbits, especially if no one else is hunting rabbits. In this case, X is joining the stag hunt, and Y is setting off on a solo rabbit hunt. It isn't as hard to get cooperative outcomes in this game as in Prisoners' Dilemma, but it takes some background trust.

## Battle of the Sexes {#sec-battle-of-sexes}

I don't particularly like the name of this game, but it is common enough that you probably should know it. @tbl-bs-standard gives a commonly used version of the game.
...
: Battle of the Sexes {#tbl-bs-standard}

@tbl-bs-abstract shows the general version, with $a > b > c$,
...
: The abstract form of Battle of the Sexes {#tbl-bs-abstract}
...
Our last game, shown in @tbl-chicken-standard, is not for the faint of heart.
...
: A familiar form of Chicken {#tbl-chicken-standard}
...
## Types of Games {#sec-types-of-games}
...
## Dominant Strategies {#sec-dominant-strategies}
...
## Dominance and Solving Games {#sec-dominance-solving}
...
The first step here lets us solve Prisoners' Dilemma. Since $Y$ dominates $X$, neither player will play $X$. So the solution to the game is that both players play $Y$. @tbl-dominance-example is another game that isn't solved by the first step, but is solved by the second step.
