# Games and Time {#sec-games-and-time}
...
## Normal Form and Extensive Form {#sec-normal-and-extensive}
...
## Backwards Induction {#sec-backwards-induction}
...
::: {.callout-note title="Exercise: Last Counter"}

Here is another game to run backwards induction on. Some counters sit on a table. $A$ and $B$ take turns removing them, with $A$ moving first. On your turn you must take either one counter or two. Whoever takes the **last** counter loses.

1. Draw the extensive form for the game that starts with four counters. Note that, unlike **Five**, the branches are not all the same length, and some nodes offer only one move.
2. Work out the normal form. Each player has four strategies, so it is a $4 \times 4$ table. How many distinct outcomes does the tree have, and how many cells does the table have?
3. Solve the game by backwards induction. Who wins?
4. Now do the same for five counters, and for six. What is the general pattern?
5. Suppose the game starts with six counters and $A$ opens badly. $B$ now finds herself at a node that a perfectly rational $A$ would never have created. Does that matter for how $B$ should play from here? Compare the case of **Five''** above.

:::

## Value of Games {#sec-value-of-games}
...
## Problems with Backwards Induction {#sec-problems-backwards-induction}

When each player only moves once, it is very plausible that each player should play their part of a subgame perfect equilibrium solution to the game. But this is less compelling when players have multiple moves. We can start to see why by looking at a game that the philosopher Robert Stalnaker [-@stalnaker1996; -@stalnaker1998] has used in a few places to argue against backwards induction reasoning. Again, it is an extensive form game. We will call it **Cooperative Caterpillar**.
...
- Alice is rational, and is signalling that she intends to pass again, in the hope that Bob will pass too and they will both do better. This is forward induction reasoning of the kind we will see in the **Money Burning Game** (@sec-money-burning), and on this reading Bob should pass.
...
There is an empirical footnote. When people play centipede games in the laboratory, they mostly do not take at the first node [@mckelvey1992]. They pass for a while and take somewhere in the middle. That does not settle anything about what is rational, but it does mean the theory is predicting something that does not happen, which is at least a reason to look at the theory again.

## Money Burning Game {#sec-money-burning}

Elchanen Ben-Porath and Eddie Dekel [-@benporathdekel1992] suggested this variant to the Battle of the Sexes game. The variation is in two steps. First, we change the payouts for the basic game to those in @tbl-bpd-bos. (Note that I'm using a lowercase letter for one of Row's options here; this is to distinguish the $d$ of down from the $D$ of Don't burn.)
...
: The version of Battle of the Sexes being used {#tbl-bpd-bos}
...
## Iterated Prisoners' Dilemma {#sec-iterated-pd}
...
But note that we had to attribute an odd belief to Player II. We had to assume that Player II is *wrong* about what Player I will play. It turns out, at least for Prisoners' Dilemma, that this is crucial. If both players are certain that both players are perfectly rational, and have no false beliefs, then the only strategy that can be rationalised is permanent defection. The proof (which I'm leaving out) is in @stalnaker1996.
