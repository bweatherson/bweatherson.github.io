...
These two theories make different assessments of @tbl-rat-no-pure. Principle 1 says that rationality rules out all but 1 strategy for each player. Principle 2 says anything goes. Which of them get it right?
...
C. Any strategy that is known to have the same expected return as a rational strategy can be rationally played.
...
: A version of @tbl-rat-payout-1 with the pure strategy equilibrium raised to 2 {#tbl-rat-payout-2}
...
: A version of Chicken {#tbl-chicken-correlated}

@tbl-chicken-correlated has two pure strategy equilibria, $Gs$ and $Sg$. It also has a rather annoying mixed strategy equilibria, where each player plays goes with probability $\nicefrac{2}{11}$, and stays otherwise, and they crash about 3% of the time.
...
: A traffic game that isn't quite Chicken {#tbl-correlated-not-chicken}
...
Given a correlated equilibrium, we can work out the expected return to each player. We do this, as always, by working out their probability of getting each outcome, multiplying the probability by the utility of that outcome, and adding up. We can also find the correlated equilibrium that maximises expected returns, but that takes some more time. Let's look again at @tbl-correlated-not-chicken.

First, let's work out the mixed strategy equilibrium for the game. We want to find a probability for Row to Go that makes Col indifferent between going and staying. Let that probability be $x$. Then we have all the following lines being equivalent.
...
: Simple coordination {#tbl-simple-coordination}
...
: Asymmetric coordination {#tbl-asymmetric-coordination}
...
Again, there are two Nash equilibria: everyone invests, and everyone does not invest. Hume's hypothesis was that in games like this, the cooperative move (in this case Investing) would be more likely in small groups than large groups. This is basically what happened in Polak's classes at Yale.
...
Neither the simple Meeting game nor Battle of the Sexes have a risk-dominant equilibrium. An asymmetric Meeting game, however, does. @tbl-coord-favor is one example.
...
: A coordination game with a favored outcome {#tbl-coord-favor}
