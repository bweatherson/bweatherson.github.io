...
## Constructing the Bets

Three bets, all fair by the group's own lights at the time they're offered.

**Bet 1 (before learning whether $B$):** The group buys a conditional bet from the bookie, "if $B$ turns out true, pays \$1 if $A$", at the group's pooled-prior conditional price of \$0.82. This is a bet that pays $1 if $A \wedge B$, pays nothing if if $A$, and the bettor gets their money back (in this case 82 cents) if $\neg A$.
...
: Net payoffs for the group for all three bets in all four world states {#tbl-dutch}
