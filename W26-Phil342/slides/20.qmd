...
4. Also, these probabilities can drift over time, in a random way. The player knows the rule for the random drift, but how any arm has drifted.
...
They are relevant to science is a straightforward way.
...
Each round, every scientist performs an experiment on whichever treatment she currently favours. In bandit terms, scientists always **exploit** rather than **explore**.
...
The mechanism is the one we saw in the toy example with A, B, C and D, but it generalises.
...
Alone, this converges on the right answer. But in a group, it can fail: the agents converge on each other's behaviour too quickly, and everyone locks in on a suboptimal theory before gathering enough evidence.

## Good for Groups, Bad for Individuals

Consider a group where each scientist has a "favourite theory." She mostly tests her favourite, but occasionally tests others. Individually, any one of these scientists might get stuck on the wrong theory forever.
...
Each scientist's stubbornness about her favourite theory looks like a bias. But that bias is what keeps the community exploring all the options. If every scientist had instead used the individually optimal strategy, they might all have converged prematurely on the same (possibly wrong) answer.
...
We'll look at how robust the Zollman effect is, what happens when diversity of belief leads to polarisation, and how outside actors can manipulate epistemic networks.
